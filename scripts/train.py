#!/usr/bin/env python3
"""
Main training script: Fine-tune ViT-B/16 on iNaturalist 2021 mini.
"""
...
def main(args):
    ...
    try:
        ...
        print("Ensure iNaturalist 2021 mini dataset is downloaded to:")
        print(f"  {Path(args.data_dir).absolute()}")
        print("\nDownload from: https://github.com/visipedia/inat_comp")
        return
    ...
    head_loader, tail_loader, head_dataset, tail_dataset = get_head_tail_loaders(
        data_dir=args.data_dir,
        batch_size=args.batch_size,
        num_workers=args.num_workers,
    )
    ...
    print(f"\nHead accuracy: {head_tail_metrics['head_accuracy']:.2f}%")
    print(f"Tail accuracy: {head_tail_metrics['tail_accuracy']:.2f}%")
    print(f"Accuracy gap: {head_tail_metrics['head_accuracy'] - head_tail_metrics['tail_accuracy']:.2f}%")
    print("\n✅ Ready for calibration: python scripts/calibrate.py")
    print("="*60 + "\n")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train ViT-B/16 on iNaturalist 2021")
