#!/usr/bin/env python3
"""
Calibration & Routing Pipeline: Temperature scaling and dynamic threshold.
"""
...
from src.data_loader import get_data_loaders, get_head_tail_loaders
from src.model import ImageClassifier
from src.calibration import CalibrationPipeline
...
def get_logits_and_labels(model, data_loader, device):
    ...
    model.eval()
    with torch.no_grad():
        for images, labels, category_ids in data_loader:
    ...
    category_ids = np.array(all_category_ids)

    return logits, labels, category_ids
...
def main(args):
    ...
    model = ImageClassifier(num_classes=train_dataset.num_classes)
    ...
    # Split validation into calibration (90%) and final test (10%)
    val_size = len(val_dataset)
    cal_size = int(0.9 * val_size)
    ...
    cal_logits, cal_labels, cal_cat_ids = get_logits_and_labels(
    ...
    test_logits, test_labels, test_cat_ids = get_logits_and_labels(
    ...
    router.set_class_info(
        head_classes=test_dataset.dataset.head_classes,
        tail_classes=test_dataset.dataset.tail_classes
    )
    ...
    head_mask = np.array([cat_id in test_dataset.dataset.head_classes for cat_id in test_cat_ids])
    tail_mask = np.array([cat_id in test_dataset.dataset.tail_classes for cat_id in test_cat_ids])
    ...
    # ========================================================================
    print("\n" + "="*60)
    print("CALIBRATION PIPELINE SUMMARY")
    print("="*60)
    print(f"\n📊 CALIBRATION METRICS")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Calibrate ViT-B/16 on iNaturalist 2021")

    parser.add_argument(
        "--data-dir",
        type=str,
        default="data/inat2021_mini",
        help="Path to iNaturalist dataset"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
